...
TEXT_FILE = "słowniki\krol-macius-pierwszy.txt"  # Plik z tekstem
CANVAS_WIDTH = 500
CANVAS_HEIGHT = 150
WORDS_NUMBER = 3
...
def load_text_snippet(word_count):
    if not os.path.exists(TEXT_FILE):
        raise FileNotFoundError(f"Plik \"{TEXT_FILE}\" nie istnieje!")
    with open(TEXT_FILE, "r", encoding="utf-8") as f:
        text = f.read()
    words = re.findall(r"\b\w+\b", text)
    if len(words) < word_count:
        raise ValueError("Plik tekstowy zawiera zbyt mało słów.")
...
class HandwritingApp:
    ...
    def update_word(self):
        self.reset_canvas()
        try:
            self.current_word = load_text_snippet(WORDS_NUMBER)  # Pobiera 3 słowa z tekstu
