...
# Here you should insert your own folder paths
# folder_paths = ["../data/output", "../data/mwoutput", "../data/poloutput", "../data/hibru", "../data/augmented"] 
...
# Add learning rate scheduler (It is optional, use it if you feel like it improves learning stability)
...
for epoch in range(starter_epoch + 1, epochs):
    ...
    for i, (input_seq, target_seq, text) in enumerate(train_loader):
        ...
        if(epoch < 2): 
            ...
            # # Define thresholds
            # # These thresholds were choosen after several tries they seem to provide better learning stability
            # # You might find moments when the model tries to write down a letter in one step, that would suggest that kappa bounds were not used
            # # There parameteres are free to tweak, they should smoothen learning proper attention, then they can be removed from loss funtion
            # # They are the most important during first epoch when model learns how many steps is required for each of the letters. 
        ...
        # Clipping by norm prevents from gradient explosion, and it doesn't significantly affect learning.
    ...
    # Step the scheduler based on validation loss (only when using scheduler)
